...
parser.add_argument('-b', '--blockdev', help='block device you wish to use .e.g. /dev/nbd2')
...
def server_read(ip, blockdev, num=2):
	try:
		...
		check_blocks = "dd if=%s of=./output%s" %(blockdev, num)
...
def repeated_connect(ip, server, blockdev, name):
    x = 0 
    while x < 1:
    #while True:
    	startTime = datetime.now()
        try:
            ...
            break
        x += 1
        print "It took %s seconds to read data" %(datetime.now() - startTime)
        
        read_time = read_time + [datetime.now() - startTime, datetime.now(), x]
...
if __name__ == '__main__':
	...
	if test_type == "repeat":
		...
		#make_log(log_file)
		report = ["Read time", "Time", "IP", "Blockdev", "Total connection time", "Connection no."]
		write_to_csv("report.csv", report)
		for ip in local_ips:
			t = threading.Thread(target=repeated_connect, args=(ip, server, blockdev, name,))
			threads.append(t)
			t.start()
		while t.is_alive():
